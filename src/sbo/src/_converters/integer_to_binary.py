...
class IntegerToBinary(QuadraticProgramConverter):
    """Convert a :class:`~src._problems.QuadraticProgram` into new one by encoding
    ...
    Examples:
        ...
        >>> from src._converters import IntegerToBinary
    ...
    def _convert_linear_coefficients_dict(
        ...
        if self._src is None:  # to fix mypy
    ...
    def interpret(self, x: Union[np.ndarray, List[float]]) -> np.ndarray:
        ...
        """
        if self._src is None or self._dst is None:  # to fix mypy
            raise ValueError("QuadraticProgram not initialized!")
