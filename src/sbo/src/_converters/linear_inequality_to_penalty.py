...
from .._problems.constraint import Constraint, ConstraintSense
...
class LinearInequalityToPenalty(QuadraticProgramConverter):
    ...
    def convert(self, problem: QuadraticProgram) -> QuadraticProgram:
        ...
        if self._should_define_penalty:
            ...
            penalty = self._penalty if self._penalty is not None else 0.0
    ...
    def _conversion_table(
        ...
        quadratic: np.ndarray = np.zeros((num_vars, num_vars), dtype=int)
        ...
        if sense == ConstraintSense.GE and rhs == num_vars - 1:
            ...
            quadratic = np.array([[0, -1], [0, 0]], dtype=int)
